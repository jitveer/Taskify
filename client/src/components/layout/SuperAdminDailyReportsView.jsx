...
function SuperAdminDailyReportsView({ color = "purple" }) {
    ...
    // Sync search if URL parameter changes
    useEffect(() => {
        if (urlSearchQuery) {
    ...
    const filteredReports = reports.filter((rep) => {
        ...
        const adminNameMatch = (rep.reportedBy?.name || rep.submittedBy?.name || "").toLowerCase().includes(query);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 pt-5">
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search admin, keyword..."
                            value={search}
                    ...
                    <div>
                        ...
                        >
                            ...
                            {departmentsList.map(dept => (
                                !["it", "hr", "sales", "csr", "interior", "accounts"].includes(dept.toLowerCase()) && (
                                    <option key={dept} value={dept.toLowerCase()}>{dept.toUpperCase()}</option>
                                )
                            ))}
                        </select>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 p-6">
                        {filteredReports.map((rep) => {
                            const submitter = rep.reportedBy || rep.submittedBy;
                            return (
                                <div
                                    key={rep._id}
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-700 font-medium line-clamp-3 leading-relaxed mt-1">
                                            {rep.description}
                                        </p>
                                    </div>
                                    ...
                                    <div className="pt-3 border-t border-slate-200/60 flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded-full bg-purple-100 text-purple-700 font-bold text-[10px] flex items-center justify-center uppercase">
                                                {submitter?.name ? submitter.name.charAt(0) : "A"}
                                            </div>
            ...
            {/* Detailed View Modal */}
            {selectedReport && (() => {
                const modalSubmitter = selectedReport.reportedBy || selectedReport.submittedBy;
                return (
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-purple-200 text-xs mt-0.5">
                                            Submitted by {modalSubmitter?.name || "Admin"} {modalSubmitter?.email ? `(${modalSubmitter.email})` : ''}
                                        </p>
                                    </div>
                                </div>
                                <button
                            ...
                            <div className="p-6 space-y-5 overflow-y-auto max-h-[65vh]">
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-100 text-xs">
                                    <div>
                                        ...
                                        <span className="font-bold text-slate-800 uppercase">{selectedReport.department}</span>
                                    </div>
                                    <div>
